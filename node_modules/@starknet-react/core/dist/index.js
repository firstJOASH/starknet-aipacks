...
function usePaymasterEstimateFees({
  ...
  const { account } = useAccount();
  const queryKey_ = useMemo7(
    () => queryKey8({ calls, options }),
    [calls, options]
  );
  const enabled = useMemo7(() => Boolean(enabled_ && calls), [enabled_, calls]);
  useInvalidateOnBlock({
    enabled: Boolean(enabled && watch),
    queryKey: queryKey_
  });
...
function usePaymasterGasTokens({
  ...
  const queryKey_ = useMemo8(() => queryKey9(), []);
  const enabled = useMemo8(() => Boolean(enabled_), [enabled_]);
  useInvalidateOnBlock({
    enabled: Boolean(enabled && watch),
    queryKey: queryKey_
  });
...
function useStarkAddress({
  ...
  const { provider } = useProvider();
  const { chain } = useNetwork();
  const enabled = useMemo9(() => Boolean(enabled_ && name), [enabled_, name]);
  return useQuery({
...
function useTransactionReceipt({
  ...
  const queryKey_ = useMemo12(() => queryKey13({ chain, hash: hash3 }), [chain, hash3]);
  const enabled = useMemo12(() => Boolean(enabled_ && hash3), [enabled_, hash3]);
  useInvalidateOnBlock({
    enabled: Boolean(enabled && watch),
    queryKey: queryKey_
  });
