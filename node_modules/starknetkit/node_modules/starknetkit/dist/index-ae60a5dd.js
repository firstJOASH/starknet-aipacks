...
import { m as U } from "./core-44275fb6.js";
import { WalletAccount as K, constants as P } from "starknet";
import { C as Z, a as S, b as R, U as E } from "./connector-d58f0dd8.js";
import { g as H } from "./trpc-63ec2300.js";
import { P as $, r as J } from "./lastConnected-c0cae438.js";
import { ArgentX as Y } from "./argentX.js";
import { A as O } from "./index-0c3fdab2.js";
const ee = {
...
const qe = ({
  ...
  return (n ? r : [
    ...
    const h = C(d), p = B(d), g = e.find((w) => w.id === p?.id);
    if (g) {
      let w, b;
      ...
      const F = t.find(
        (y) => y.id === (g.id === "argentMobile" ? "argentX" : p?.id)
      )?.downloads;
      return {
        name: b,
        id: g.id,
...
class Re extends Z {
  ...
  async account(t) {
    ...
    return new K(t, this._wallet, void 0, i[0]);
  }
  async chainId() {
    if (!this._wallet)
      throw new S();
  ...
  async ensureWallet(t) {
    const { getStarknetWindowObject: i } = await import("./index-3533a280.js").then((y) => y.i), { chainId: n, projectId: c, dappName: a, description: f, url: l, icons: d, rpcUrl: h } = this._options, p = re(), g = h ?? (!n || n === P.NetworkName.SN_MAIN ? p.mainnet : p.testnet), v = (await U.getDiscoveryWallets()).map((y) => y.id.toLowerCase() === "argentx" ? {
      ...y,
    ...
    if (!b)
      throw new E();
    this._wallet = b;
    const F = this._wallet;
    await F.enable(), F.client.on("session_delete", () => {
